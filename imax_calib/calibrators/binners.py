...
class HistogramBinninerCW(BaseCalibrator):
    def __init__(self, cfg, scaler_obj=None, load_params=False, **kwargs):
        ...
        if self.scaler_obj is None:
            assert self.binning_stage == "raw", "If no scaler_obj is provided then binning stage can only be RAW logodds"
        ...
        if len(self.parameter_list) > 0:
            ...
            for class_idx, histbin_c in enumerate(self.list_binners):
                ...
                for key in self.parameter_list:
                    data_to_save["class_%d" % (class_idx)][key] = getattr(histbin_c, key)
    ...
    def get_calib_parameters(self):
        ...
        for param_name in self.parameter_list:
            ...
            all_params["bin_representations"] = []
            for histbin_c in self.list_binners:
                all_params[param_name].append(getattr(histbin_c, param_name))
                if histbin_c.binning_repr_scheme == "pred_prob_based":
                    all_params["bin_representations"].append(all_params["bin_representations_PPB"])
                elif histbin_c.binning_repr_scheme == "sample_based":
                    all_params["bin_representations"].append(all_params["bin_representations_SB"])
            all_params[param_name] = np.array(all_params[param_name])
            all_params["bin_representations"] = np.array(all_params["bin_representations"])
        return all_params
...
class HistogramBinninerTop1(BaseCalibrator):
    ...
    def save_params(self, fpath):
        ...
        if len(self.parameter_list) > 0:
            data_to_save = {}
            for key in self.parameter_list:
                data_to_save[key] = getattr(self.histbin_c, key)
            io.deepdish_write(fpath, data_to_save)
